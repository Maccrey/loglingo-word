...
- **AI 이성친구 대화 채팅** (구독자 전용, 하루 30분 / 연장 $1)
  - 설정의 성별(gender)에 따라 반대 이성 AI 캐릭터와 대화
  - 학습어·모국어·레벨을 프롬프트에 반영한 자연스러운 회화
  - 잘못된 문장 교정 + "원어민이라면 이렇게 말해요" 어드바이스

## 핵심 규칙 요약

- 기본 시작 캐릭터는 새끼 고양이 1마리다.
- 하루 15분 이상 학습하면 기본 돌봄에 필요한 포인트를 안정적으로 확보할 수 있어야 한다.
- 하루에 한 번씩 `밥주기`, `씻기기`, `놀아주기`를 해주도록 유도한다.
- 학습하지 않으면 12시간 후 배고픔 상태가 시작된다.
- 먹이를 준 뒤 24시간이 지나면 냄새 상태가 시작된다.
- 마지막으로 놀아준 뒤 3시간이 지나면 스트레스가 쌓이기 시작한다.
- 마지막 급식 후 7일이 지나면 고양이는 사망한다.
- 하루 동안 아무 돌봄 방문이 없으면 치료가 필요한 상태가 된다.
- 치료가 필요한 상태에서 3일 동안 치료하지 않으면 고양이는 사망한다.
- 약 또는 주사를 사용하면 질병 상태를 회복할 수 있고, 치료 필요 상태도 해제된다.
- 고양이가 죽으면 성장 단계에 맞는 dead 이미지를 보여주고 `고양이 다시 키우기` 버튼으로 처음부터 다시 시작할 수 있다.
...
- `밥주기`, `씻기기`, `놀아주기`는 하루 기준 완료 여부를 따로 추적한다.
- 홈 카드와 상세 화면은 `오늘 완료` / `오늘 필요` 형태로 현재 필요한 돌봄을 보여준다.
- 하루 동안 고양이 돌봄 방문이 없으면 `치료하기`가 최우선 돌봄으로 바뀐다.
- 치료 필요 상태가 된 뒤에는 일반 돌봄만으로는 회복되지 않고 `치료하기`를 해야 한다.
- 치료 없이 3일이 지나면 고양이는 죽은 상태가 되며 일반 돌봄 버튼은 숨긴다.
- 죽은 상태에서는 성장 단계별 dead 이미지를 표시하고 `고양이 다시 키우기` 버튼만 노출한다.
- `고양이 다시 키우기`를 누르면 고양이 상태와 돌봄 기록은 처음 상태로 초기화된다.
