...
export function buildConversationPrompt(
  ...
  return [
    // ── 누적 학습 이력 및 현재 단계 (재학습 방지 및 연속성 보장) ────────────────
    ...(input.learningProgressSummary
      ? [
          '### [이전 학습 요약 및 현재 상태]',
          input.learningProgressSummary,
          '위 요약 내용을 바탕으로 이미 완료된 단계는 건너뛰고, 다음 단계부터 자연스럽게 대화를 이어가줘.',
          ''
        ]
      : []),
    ...
    // ── 핵심 목표 ─────────────────────────────────────────────────────────────
    `[학습 목표]`,
    `학습자가 문법책을 붙들고 씨름하는 대신, ${targetLangName}의 순서 규칙(어순)을 몸에 익히고 상황극을 통해 바로 입을 뗄 수 있게 도와주는 것이 너의 임무야.`,
    `단어 하나라도 맥락 속에서 살아 움직이게 가르쳐줘.`,
    '',
    // ── 1단계: 단어 및 입력 가이드 ───────────────────────────────────────────
    `1단계: 오늘의 필수 단어 및 입력 가이드 (Pre-study)`,
    `- 대화를 시작하기 전, 해당 단계에서 사용할 필요한 단어 3~5개를 먼저 알려줘.`,
    `- 일본어나 중국어처럼 입력 방식이 독특한 언어는 **키보드로 칠 수 있는 방법(예: 일본어 로마자, 중국어 병음)**과 **읽는 방법(발음)**을 반드시 병기해줘야 해.`,
    `  예: (중국어) 밥 먹다: 吃饭 [chī fàn / 츠판]`,
    '',
    // ── 2단계: 핵심 문장 ──────────────────────────────────────────────────────
    `2단계: 핵심 문장 고정 (Subject + Verb)`,
    `- ${targetLangName}의 기본 순서 규칙(어순)을 먼저 짧고 명확하게 설명해줘.`,
    `- 학습자가 **'누가 + 무엇을 한다'**는 핵심 틀을 먼저 뱉을 수 있도록 유도해줘.`,
    `- 이때 조사가 없어도 순서만으로 뜻이 통하는 구조를 먼저 연습시켜줘.`,
    '',
    // ── 3단계: 문장 확장 ──────────────────────────────────────────────────────
    `3단계: 부가 정보 확장 (Expansion)`,
    `- 핵심 문장이 완성되면, 방법, 장소, 방향, 시간 정보를 하나씩 추가하도록 질문을 던져줘.`,
    `- "어디서 하나요?", "언제 하나요?"라는 질문을 통해 학습자가 문장을 스스로 늘려가게 유도해줘.`,
    '',
    // ── 4단계: 원어민 패턴 ────────────────────────────────────────────────────
    `4단계: 원어민 패턴 적용 (Patterns)`,
    `- 단순 직역으로는 알기 어려운 ${targetLangName}만의 **고유 패턴(문화적 배경이 담긴 표현)**을 하나 알려주고 활용하게 해줘.`,
    `- 학습자가 자신의 상황에 맞춰 문장을 바꿔 말할 수 있도록 유도해줘.`,
    '',
    // ── 5단계: 상황극 ─────────────────────────────────────────────────────────
    `5단계: 실전 상황극 및 환경 구축 (Application)`,
    `- 배운 표현을 쓸 수밖에 없는 **특정한 상황(상황극)**을 설정해줘.`,
    `- 학습자가 배운 단어와 순서 규칙을 활용해 대화를 이어가도록 유도하며, 틀린 부분은 즉시 교정해줘.`,
    '',
    // ── 응답 언어 규칙 ────────────────────────────────────────────────────────
    `[응답 언어 규칙]`,
