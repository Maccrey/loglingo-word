...
### 3.1 고빈도 데이터 쓰기 최적화 (Write-Behind / Debouncing)

- **대상 데이터**: 고양이 상태(Cat status), 포인트 렛저(Point Ledger), 학습 진척도(Vocab Progress).
- **결정 사항**: 사용자의 상호작용(카드 학습, 돌봄 액션) 발생 시 즉시 Firestore에 쓰지 않고, **30초 디바운싱(Debouncing)** 타이머를 사용하여 변경사항을 메모리에 모았다가 한 번에 전송합니다.
- **이유**: Firestore의 쓰기 쿼터(Quota)를 보호하고 네트워크 비용을 절감하여, 대규모 사용자 접속 시에도 안정적인 서비스를 유지하기 위함입니다.
- **데이터 일관성 보장**:
  - `flushSync()` 또는 `flushSaveLearningState()` 함수를 제공하여 세션 종료, 결제 완료, 앱 언마운트 등 데이터 유실이 우려되는 시점에는 즉시 강제 동기화를 수행합니다.
  - 클라이언트 사이드 로컬 스토리지(`LocalStorage`)를 1차 저장소로 사용하여 동기화 전에도 사용자에게 즉각적인 피드백을 제공합니다.

### 3.2 로컬 우선 아키텍처 (Local-First Priority)

- 모든 변화는 로컬 상태와 저장소에 즉시 반영되며, Firestore는 영속적 백업 및 멀티 디바이스 동기화 용도로 사용합니다.

## 4. AI 서비스 및 모델 관리

### 4.1 AI 모델 환경 변수화 (Configurable AI Models)

- **대상**: OpenAI ChatGPT 모델 (`gpt-4o`, `gpt-4o-mini` 등)
- **결정 사항**: AI 모델명을 코드에 하드코딩하지 않고, `OPENAI_API_MODEL` 환경 변수를 통해 관리한다. 별도의 설정이 없을 경우 기본값으로 `gpt-4o-mini`를 사용한다.
- **이유**: 운영자가 앱 배포 없이 `.env` 파일 수정만으로 비용 최적화(소형 모델) 또는 성능 최적화(플래그십 모델)를 선택할 수 있게 하기 위함이다.
- **구조**: `packages/shared/src/env.ts`의 `envSchema`에서 타입을 검증하고, `services/ai/src/openai-client.ts`에서 이를 주입받아 사용한다.
