...
describe('Cat Status Transition Engine', () => {
  ...
  describe('T2-1 to T2-3: Basic Status Functions', () => {
    ...
    it('should derive stress state boundaries from play neglect time', () => {
      ...
      const sickStressCat = createMockCat(0, 0, 25);
  ...
  describe('T2-4 & T2-5: Comprehensive Status Calculation', () => {
    ...
    it('should keep hungry, smelly, and stressed only before the 24-hour sickness threshold', () => {
      const hungryCat = createMockCat(13, 10, 10);
      expect(calculateCatStatus(hungryCat, Date.now(), mockEnv)).toBe('hungry');

      const smellyCat = createMockCat(5, 24, 10);
      expect(calculateCatStatus(smellyCat, Date.now(), mockEnv)).toBe('sick');

      const stressedCat = createMockCat(2, 2, 13);
      expect(calculateCatStatus(stressedCat, Date.now(), mockEnv)).toBe('stressed');
    });

    it('should return sick when abandoned past sick threshold', () => {
      const cat = createMockCat(25, 10, 10);
    ...
    it('should trigger severity from the specific neglected care dimension', () => {
      ...
      const cat = createMockCat(10, 10, 25);
      expect(calculateCatStatus(cat, Date.now(), mockEnv)).toBe('sick');
    });
    ...
    it('should expose a consistent cat state snapshot', () => {
      const cat = createMockCat(13, 10, 13);
      ...
      expect(snapshot.isSmelly).toBe(false);
